...
int bb_getattr(const char *path, struct stat *statbuf)
{
    ...
    log_stat(statbuf);
    if (strcmp(get_filename(path), "buddy.txt") == 0)
    {
        statbuf->st_mode = S_IFREG | 0444; // source: https://stackoverflow.com/questions/40163270/what-is-s-isreg-and-what-does-it-do
        statbuf->st_nlink = 1;
        statbuf->st_size = strlen(FILE_CONTENT);
        return 0;
    }
...
int bb_open(const char *path, struct fuse_file_info *fi)
{
    ...
    if ((fd < 0 && strcmp(filename, "buddy.txt") != 0) || strstr(filename, SECRET) == filename)
               ...
               struct fuse_file_info *fi)
{
    ...
    do
    {
        const char *filename = get_filename(de->d_name);
        if (strstr(filename, SECRET) != filename)
        {
            log_msg("calling filler with name %s\n", de->d_name);
            if (strcmp(de->d_name, "buddy.txt") == 0)
            {
                containsBuddy = 1;
            }
    ...
    if (!containsBuddy)
    {
        log_msg("calling filler with name buddy.txt\n");
        filler(buf, "buddy.txt", NULL, 0);
    }
