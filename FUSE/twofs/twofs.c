...
int bb_getattr(const char *path, struct stat *statbuf)
{
    ...
    if (strcmp(get_filename(path), FILE_1) == 0 || strcmp(get_filename(path), FILE_2) == 0) // file normal file
    {
        statbuf->st_mode = S_IFREG | 0666;
        statbuf->st_nlink = 1;
        statbuf->st_size = strcmp(get_filename(path), FILE_1) == 0 ? file1_size : file2_size;
        statbuf->st_uid = 0;
        statbuf->st_gid = 0;
...
int bb_open(const char *path, struct fuse_file_info *fi)
{
    ...
    if (fd < 0 && strcmp(get_filename(path), FILE_1) != 0 && strcmp(get_filename(path), FILE_2) != 0)
...
int bb_read(const char *path, char *buf, size_t size, off_t offset, struct fuse_file_info *fi)
{
    ...
    char *fpath = BB_DATA->rootdir;
    int fd = 0;
    size_t max_size;
    if (strcmp(get_filename(path), FILE_1) == 0)
    {
        log_msg("go into file1, %s", path);
        fd = open(fpath, O_RDONLY);
        log_msg("%ul, %ul, %ul\n", offset, file1_size, block_size);
        max_size = file1_size;
    }
    else if (strcmp(get_filename(path), FILE_2) == 0)
    {
        log_msg("go into file2, %s", path);
        fd = open(fpath, O_RDONLY);
        offset += file1_size;
        log_msg("%ul, %ul, %ul\n", offset, file2_size, block_size);
        max_size = block_size;
    }
    else
    {
             ...
             struct fuse_file_info *fi)
{
    ...
    if (offset < max_size)
    {
        ...
        log_msg("%d, %ul, %ld\n", fd, size, offset);
        int check = pwrite(fd, buf, size, offset);
        if (check < 0)
        {
               ...
               struct fuse_file_info *fi)
{
    ...
    filler(buf, ".", NULL, 0);
    filler(buf, "..", NULL, 0);
    filler(buf, FILE_1, NULL, 0);
    filler(buf, FILE_2, NULL, 0);
...
int main(int argc, char *argv[])
{
    ...
    if (stat(block_path, &file_info) != 0)
    {
        perror("block stat error");
        abort();
    }

    // calculate the size of file1 and file2
    block_size = file_info.st_size;
    file1_size = block_size / 2;
    file2_size = file_info.st_size - file1_size;
